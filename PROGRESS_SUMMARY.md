...
- ✅ API Settings 后端验证（`.env` 配置管理）
- ✅ API Settings 前端 UI（Provider 选择、客户端验证）
- ✅ Milvus schema 修复（维度 1024→384，JSON→VARCHAR）
- ✅ Docker 生产部署配置
  - 后端多阶段构建（python:3.10-slim）
  - 前端多阶段构建（Node.js 20 + nginx 1.27）
  - 非 root 用户运行，健康检查
- ✅ 前端会话管理侧边栏改进
...
| 2026-05-08 | 基础 RAG API 开发 | ✅ |
| 2026-05-24 | API Settings + Docker 部署 | ✅ |
| 2026-05-24 | 前端会话管理完善 | ✅ |
| 2026-05-30 | 第一批数据导入 | 🎯 目标 |
...
9. ✅ Docker 生产部署（后端/前端）
10. ✅ 前端会话管理侧边栏
11. ✅ Milvus 客户端 server/lite 自动切换
