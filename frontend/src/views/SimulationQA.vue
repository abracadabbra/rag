  ...
  <div class="simulation-qa">
    ...
    <header class="header">
      <div class="header-left">
        ...
        <div class="title-group"><h1>仿真解读</h1><span class="subtitle">解读仿真结果与分析模型表现</span></div>
      </div>
...
import { querySimulation } from '../services/api.js'
...
const suggestions = ['最新一次仿真结果如何？', 'A 模型和 B 模型哪个表现更好？', '仿真的 KS 值是多少？']
...
const handleClarificationSelect = async (option) => {
  ...
  try { const result = await querySimulation({ query: '', session_id: sessionId.value, clarification_choice: option }); messages.value.push({ role: 'assistant', answer: result.answer, sources: result.sources }) } catch (e) { error.value = e.message || '查询失败' }
}
...
<style scoped>
.simulation-qa { max-width: 960px; margin: 0 auto; padding: 0; min-height: 100vh; display: flex; flex-direction: column; position: relative; z-index: 1; }
