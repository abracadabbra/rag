...
@router.post(
    ...
    summary="模型卡片问答",
    description="基于 RAG 检索模型卡片文档并生成答案"
)
async def query_model_cards(request: QueryRequest):
    """
    模型卡片问答
    ...
    ```json
    {
        "answer": "客服机器人模型部署在 10.0.1.100，Docker 镜像...",
        "sources": [
            {
                "score": 0.95,
                "content_preview": "部署服务器: 10.0.1.100...",
                "rule_id": "M001",
                "rule_name": "客服机器人模型"
            }
        ],
    ...
    logger.info(
        f"[{request_id}] 收到模型卡片查询请求 - Query: {request.query[:50]}..., "
